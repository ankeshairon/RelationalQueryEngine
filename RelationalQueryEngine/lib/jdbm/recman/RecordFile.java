...
//    /** maximal file size not rounded to block size */
    ...
    void close() throws IOException {
        ...
        if (!dirty.isEmpty()) {
            System.out.println("ERROR: dirty blocks at finish time");
            showList(dirty.valuesIterator());
            throw new Error("Dirty blocks at finish time");
        }
        if (!inUse.isEmpty()) {
            System.out.println("ERROR: inUse blocks at finish time");
            showList(inUse.valuesIterator());
            throw new Error("inUse blocks at finish time");
        }
